...
server <- function(input, output) {
  ...
  #Create separate datasets for segments 1,2,3,4
  ...
  seg4 <- reactive({ tidy.tbl2()%>%
      filter(segID=="s4")%>%
      select(segType, Number, Power, C0, C1, L1, R1, fs, Gamma)%>%
      arrange(Power)
  })
  #Create final summary dataset with calculated metrics
  final.data <- reactive({ data.tbl <- data.frame(RMS.deltaV = seg3()$RMS.deltaV)
  #delta fs and gamma
  #this part of the code detects which segment is the baseline 
  #and calculates gamma and frequency shifts accordingly
  data.tbl <-
    if(seg1()$segType[1] == "B" & seg4()$segType[1] != "B"){
      mutate(data.tbl,
             deltaFs = seg3()$fs - seg1()$fs,
             deltaGamma = seg3()$Gamma - seg1()$Gamma)
    } else if(seg4()$segType[1] == "B" & seg1()$segType[1] != "B"){
      mutate(data.tbl,
             deltaFs = seg3()$fs - seg4()$fs,
             deltaGamma = seg3()$Gamma - seg4()$Gamma)
    } else if(seg1()$segType[1] == "B" & seg4()$segType[1] == "B"){
      mutate(data.tbl,
             deltaFs = seg3()$fs - (seg1()$fs+seg4()$fs)/2,
             deltaGamma = seg3()$Gamma - (seg1()$Gamma+seg4()$Gamma)/2)
    }
  data.tbl <- data.tbl%>%
    #quality factor
    mutate(Q = 2*pi*seg3()$fs*(seg3()$L1/seg3()$R1))%>%
