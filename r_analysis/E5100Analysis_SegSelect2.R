...
ui <- fluidPage(
  ...
  fluidRow(column(width=4,offset=0,
                  div(style = "height:50px;", "by: Lucas Kramarczuk & Eskil Irgens"))),
  #File input
  fluidRow(column(width=4,
                  fileInput("upload", NULL, buttonLabel = "Choose File:", multiple = FALSE))),
  #Row 0 - Selecting which segments to analyze
  fluidRow(column(width=8,plotOutput("Frequency"), plotOutput("Bandwith")),
           column(width = 4, numericInput("Baseline1","Baseline 1 (red)",1),numericInput("Test_seg","Test segment (green)",2),numericInput("Baseline2","Baseline 2 (orange)",3),
                  checkboxInput("BaselineComparison", "Include Baseline Comparison"))),
  #Helper text
  ...
  fluidRow(
    column(width = 3,
           verbatimTextOutput("stiffVecBaseline1"))),
  ...
  fluidRow(
    column(width = 6,
           plotOutput("damping", brush = "plot_brush_damp")),
    column(width = 6,
           plotOutput("elastic", brush = "plot_brush_elas"))),
...
#separating out segments
segment_extraction <- function(segcount,dataset, segID) {
  sx<-paste("s",as.character(segcount),sep = "")
  dataset <- dataset()%>%
    mutate(segIDnum = as.numeric(gsub("\\D", "", segID)))
  seg <- dataset%>%
    filter(segIDnum == segcount) %>%
...
server <- function(input, output) {
  ...
  #delta fs and gamma
  #Calculates gamma and frequency shifts from baseline average
  data.tbl <-
    mutate(data.tbl,
           deltaFs = segextract()$Test_seg$fs - (segextract()$Baseline2$fs + segextract()$Baseline1$fs)/2,
  ...
  final.data.B1 <- reactive({ 
    if (input$BaselineComparison == TRUE) {
    data.tbl <- data.frame(RMS.deltaV = segextract()$Test_seg$RMS.deltaV)
  #delta fs and gamma
  #Calculates gamma and frequency shifts from baseline average
  data.tbl <-
    mutate(data.tbl,
           deltaFs = segextract()$Test_seg$fs - segextract()$Baseline1$fs,
           deltaGamma = segextract()$Test_seg$Gamma - segextract()$Baseline1$Gamma)
  data.tbl <- data.tbl%>%
    #quality factor
    mutate(Q = 2*pi*segextract()$Test_seg$fs*(segextract()$Test_seg$L1/segextract()$Test_seg$R1))
  ...
  data.tbl <-
    mutate(data.tbl,
           deltaFs = segextract()$Test_seg$fs - segextract()$Baseline2$fs,
           deltaGamma = segextract()$Test_seg$Gamma - segextract()$Baseline2$Gamma)
  data.tbl <- data.tbl%>%
    #quality factor
    mutate(Q = 2*pi*segextract()$Test_seg$fs*(segextract()$Test_seg$L1/segextract()$Test_seg$R1))
  ...
  #First 5 stiffness data points of Baseline 1
  first5Baseline1 <- reactive ({ 
    deltaFs <- segextract()$Baseline1$fs - segextract()$Baseline2$fs
    data.frame(Amp=final.data()$Amp[c(1:5)],
               Ki.elas=2*(pi^2)*z.quartz*area.effective*deltaFs[c(1:5)]/1000)
  })
  
  #Frequency and bandwith output of whole data set
  output$Frequency <- renderPlot({
    ggplot(tidy.tbl2())+
      geom_point(aes(x = Number, y = fs), color = "blue")+
      geom_point(data = segextract()$Baseline1, aes(x = Number, y = fs), color = "red")+
      geom_point(data = segextract()$Test_seg, aes(x = Number, y = fs), color = "green")+
      geom_point(data = segextract()$Baseline2, aes(x = Number, y = fs), color = "orange")+
      labs(title = "Frequency all segments",
           x = "Index",
           y = "Frequency")+
  ...
  output$Bandwith <- renderPlot({
    ggplot(tidy.tbl2())+
      ...
      geom_point(data = segextract()$Baseline1, aes(x = Number, y = Gamma), color = "red")+
      geom_point(data = segextract()$Test_seg, aes(x = Number, y = Gamma), color = "green")+
      geom_point(data = segextract()$Baseline2, aes(x = Number, y = Gamma), color = "orange")+
      labs(title = "Bandwith all segments",
           x = "Index",
           y = "Bandwith")+
      #edit text sizes for title and axes labels
      theme(plot.title = element_text(size=24,face="bold"),
  ...
  output$stiffness <- renderPlot({
    ggplot(final.data())+
      geom_point(data=final.data.B1(), aes(Amp,Ki.elas), color = "pink")+
      geom_point(data=final.data.B2(), aes(Amp,Ki.elas), color = "purple")+
      geom_point(aes(x = Amp, y = Ki.elas), color = "blue")+
      geom_point(data=first5(), aes(Amp,Ki.elas),color="green")+
      geom_point(data=first5Baseline1(), aes(Amp,Ki.elas),color="orange")+
  ...
  #Segment 1 stiffness  
  stiffness.Baseline1.vec <- reactive({
    data <- first5Baseline1()$Ki.elas
    mean <- round(mean(data),3)
    sd <- round(sd(data),3)
    paste("Orange Data (Baseline 1):","\nMean: ",mean,"\nStDev:",sd)
  })
  
  output$stiffVecBaseline1 <- reactive ({ 
    stiffness.Baseline1.vec()
  })
  
  #Code for delta Q inverse plot
  output$deltaQinv <- renderPlot({
    ggplot(final.data())+
      geom_point(data=final.data.B1(), aes(Amp,TwoPi.fb), color = "pink")+
      geom_point(data=final.data.B2(), aes(Amp,TwoPi.fb), color = "purple")+
  ...
  #Code for outputting damping force maximum
  observeEvent(input$plot_brush_damp,
               {output$info3 <- renderText({
                 paste0("Mean Force:     ", round(damping.friction.force(),3),
                        "\nMean Amplitude: ", round(x.damp(),3),
                        "\nDamping COF:     ", damping.COF())
               })})
  #Code for outputting elastic force maximum
  observeEvent(input$plot_brush_elas,
  ...
  brushed.tbl.damp <- reactive({
    brushed <- brushedPoints(final.data(), input$plot_brush_damp)
    x <- brushed$Amp
    y <- brushed$F.damp
    data_frame(Amp = x,F.damp=y)
  })
  #Create data frame of highlighted points for elastic force
  brushed.tbl.stat <- reactive ({ 
    brushed <- brushedPoints(final.data(), input$plot_brush_elas)
    x <- brushed$Amp
    y <- brushed$F.elas
  ...
  x.stat <- reactive({ mean(brushed.tbl.stat()$Amp )})
  static.friction.force <- reactive({ mean(brushed.tbl.stat()$F.elas )})
  # Code for highlighting the brushed points on damping force plot
  x.damp <- reactive({ mean(brushed.tbl.damp()$Amp)})
  damping.friction.force <- reactive({ mean(brushed.tbl.damp()$F.damp )})
  
  #StdDev of highlighted points for elastic force
  xsd <- reactive({ sd(brushed.tbl.stat()$Amp )})
  ysd <- reactive({ sd(brushed.tbl.stat()$F.elas )})
  
  #Code for average elastic force plot
  output$elastic <- renderPlot({
    ggplot()+
      geom_point(data=final.data.B1(), aes(Amp,F.elas), color = "pink")+
      geom_point(data=final.data.B2(), aes(Amp,F.elas), color = "purple")+
      geom_point(data=final.data(),aes(x = Amp, y = F.elas),color="Blue")+
      geom_point(data=brushed.tbl.stat(), aes(Amp,F.elas),color="Red")+
  ...
  #Code for extracting damping COF from damping force brushed points
  damping.COF <- reactive({
    val <- round(damping.friction.force()/normal.load(),3)
  })
  
  #Code for average damping force plot
  output$damping <- renderPlot({
    ggplot()+
      geom_point(data=final.data.B1(), aes(Amp,F.damp), color = "pink")+
      geom_point(data=final.data.B2(), aes(Amp,F.damp), color = "purple")+
      geom_point(data=final.data(),aes(x = Amp, y = F.damp), color = "blue")+
      geom_point(data=brushed.tbl.damp(),aes(x = Amp, y = F.damp), color = "red")+
  ...
  output$eStored <- renderPlot({
    ggplot(final.data())+
      geom_point(data=final.data.B1(), aes(Amp,deltaE.elas), color = "pink")+
      geom_point(data=final.data.B2(), aes(Amp,deltaE.elas), color = "purple")+
  ...
  output$lossTangent <- renderPlot({
    ggplot(final.data())+
      geom_point(data=final.data.B1(), aes(Amp,delGamma.delFs), color = "pink")+
      geom_point(data=final.data.B2(), aes(Amp,delGamma.delFs), color = "purple")+
  ...
  output$ELossGlobal <- renderPlot({
    ggplot(final.data())+
      geom_point(data=final.data.B1(), aes(Amp,deltaE.damp), color = "pink")+
      geom_point(data=final.data.B2(), aes(Amp,deltaE.damp), color = "purple")+
  ...
  filtered.data.B1 <- reactive({ #reactive object updates every time the user changes the slider
    req(input$sliderRange)
    filter(final.data.B1(), Amp>input$sliderRange[1] & Amp<input$sliderRange[2])
  })
  filtered.data.B2 <- reactive({ #reactive object updates every time the user changes the slider
    req(input$sliderRange)
    filter(final.data.B2(), Amp>input$sliderRange[1] & Amp<input$sliderRange[2])
  })
  ...
  output$ELossLinear <- renderPlot ({
    ggplot(filtered.data())+ #use the reactive filtered data for plotting
      geom_point(data = filtered.data.B1(), aes(Amp,deltaE.damp), color = "pink")+
      geom_point(data = filtered.data.B2(), aes(Amp,deltaE.damp), color = "purple")+
      geom_point(aes(x = Amp, y = deltaE.damp), color = "blue")+
  ...
  results.data <- reactive ({
    names <- c("Filename","Normal.Load (uN)","Kinetic.Friction.Force (uN)",
               "COF.Kinetic","Damping.Force","COF.D","Static.Friction.Force (uN)","COF.Static",
    ...
    vals <- reactive ({
      c(input$upload$name,normal.load(),kinetic.friction.force(),cof(),
        x.damp(),damping.COF(),x.stat(),static.COF(),xsd(),m(),b(),rsquared(),sliderMin(),
